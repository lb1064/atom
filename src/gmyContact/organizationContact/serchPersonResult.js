...
new (function(){
	...
	this.mod.defaultView = function(params){
		if(params.data){
			if(params.data.key.indexOf('\'')!=-1||params.data.key.indexOf('\.')!=-1||params.data.key.indexOf('\*')!=-1){
				me.mod.main.alert({
	...
	this.openGrid = function(params){
		gridPath = Atom.clone(params.trackPath);
		...
		me.mod.main.open({
			id:'orgPersonGrid_id'+new Date().getTime(),
			xtype:'grid',
			mode:'loop',
			checkbox:true,
			currentPage:1,
			...
			searchConfig:{
				...
				handler:function(e,obj){
					...
					o.key = obj.getValue();
					o.type = '2';
					o.groupUuid = gridPath[gridPath.length-1].value;
					me.mod.defaultView({
						data:o,
						trackPath:gridPath
					});
...
//					var trackpath = data.path.slice(0);				
					...
					for(var i=1;i<trackPath.length-1;i++){
					...
					trackPath[0].handler = function(e,o){
						me.mod.main.remoteOpen({
...
//					{name:'删除',value:'删除',handler:me.delGroupPerson,exp:'all'}
			...
			colnums:[
				...
				{header:'名称',width:100,cursor:'pointer',mapping:'name',textAlign:'left',handler:me.viewPerson},
				{header:'手机',width:100,renderer:me.renderPhone,mapping:'phones',textAlign:'left'},
				...
				{header:'邮箱',width:'max',mapping:'intraEmail',textAlign:'left'},
