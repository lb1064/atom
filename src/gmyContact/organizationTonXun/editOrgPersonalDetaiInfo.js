...
new (function(){
	...
	//	var persContactName = params.persContactName;//我的通讯录中子文件夹得名字
		//alert('persContactName::'+persContactName);
		uuid = params.uuid;//联系人的uuid
		tspath = params.tspath;  //#
		nodeName = params.nodeName;
		tspath.push({name:nodeName,uuid:''});		
		//oldparentContactName = params.oldparentContactName;//上级的上级通讯录名字（通讯录管理列表中的通讯论名）
		//alert('oldparentContactName::'+oldparentContactName);
		//lastPersContactName = params.lastPersContactName //查看联系人详情的第三个节点
		me.mod.main.open({
			id:'eidt_OrgPersonDetailContact_InfoId2',
			xtype:'form',
			mode:'loop',
			checkbox:false,
			usePage:false,
			...
			store:{
				...
				success:function(data,mod){
					...
					var form = me.mod.main.get('eidt_OrgPersonDetailContact_InfoId2');
					var isSystemUsereditId = form.getByItemId('isSystemUsereditId_forEditForm');
			...
			fields:[
				...
				{height:36,cols:[//165
					...
					{xtype:'blank',width:5},
					{xtype:'text',textAlign:'left',value:'',leftHidden:true,width:'max',name:'email',itemId:'editEmailText_itemId',onTextChange:me.diaoBackPaltToCheck_OrgINedit},
				...
				{height:36,cols:[//165
					...
					{xtype:'blank',width:5},
					{xtype:'text',textAlign:'left',value:'',leftHidden:true,width:'max',name:'phone',itemId:'phoneText_itemId',onTextChange:me.diaoBackPaltToCheck_OrgINedit},
					{xtype:'blank',width:20}					
				]},
				{height:20,cols:[
						{xtype:'blank',width:20},
				...
				{height:30,cols:[
					...
					{xtype:'checkbox',width:200,name:'isSystemUser',itemId:'isSystemUsereditId_forEditForm',displayValue:'该用户为系统用户',value:'1'},
					{xtype:'blank',width:'max'}				
				]}
			],
			hiddens:[
	...
	this.diaoBackPaltToCheck_OrgINedit = function(){//吊一个后台服务看是不是系统用户
		var form = me.mod.main.get('eidt_OrgPersonDetailContact_InfoId2'); 
		var emailText = form.getByItemId('editEmailText_itemId');//邮箱
		var phoneTextItem_id = form.getByItemId('phoneText_itemId');
		var isSystemUserItemId = form.getByItemId('isSystemUsereditId_forEditForm');
	...
	this.createBaseicInfoPanel = function(obj){
		...
		nameText4 = new Gframe.controls.TextField({leftHidden:true,width:'max',name:'name',value:obj.name,onTextChange:me.diaoBackPaltToCheck_OrgINedit});
