new (function(){
	...
	this.mod.defaultView = function(params){
		...
		//console.log('reloadExpendView:::',reloadExpendView);
		fn = params.fn;
		//groupUuid = params.parentUuid;
		me.mod.main.open({
			key:'departmetList_id',
			id:'departmetList_id'+groupUuid,
			xtype:'grid',
			mode:'loop',
			checkbox:false,
			usePage:true,
...
//				}
			...
			colnums:[
					{header:'名称',textAlign:'center',width:200,cursor:'pointer',mapping:'name',handler:me.openTwo},
	...
	//打开一个页面
	this.openTwo = function(e,da){
		if(!da.sys){//不是系统组才可以查看
			var params = {uuid:da.uuid,reloadExpendView:reloadExpendView};
	...
	this.openEditPage = function(e,o){
		...
		if(!d.sys){
			me.mod.main.remoteOpen({
				url:'modules/guserManager/departments/editorDept.js',
				params:{
